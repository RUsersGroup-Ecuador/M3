# setwd("C:/Users/crbdl/Downloads/")
# datos.suicidio <-read.csv("SUICIDIO.csv", sep=";", header = TRUE,
#                           dec = ",")

datos.suicidio <-read.csv("https://raw.githubusercontent.com/RUsersGroup-Ecuador/M3/master/datatemp/SUICIDIO.csv", sep=";", header = TRUE,
                          dec = ",")
...
library(DT)
ui <- fluidPage(
  tabsetPanel(#type = "infor",
              type = "tabs",
              tabPanel("PRIMERO",
                       sidebarLayout(
                         sidebarPanel(
                           sliderInput("n",
                                       "Registros", min = 1, max = 10000, value = 1000),
                         ...
                         mainPanel(
                           tabsetPanel(type="pills",
                             #tabPanel("Tabla",datos.suicidioTableOutput("tabla"),
...
server <- function(input, output) {
  ...
  output$tabla <- renderDataTable({
    ...
    datatable(head(datos.suicidio[,c("PAISES", "PIB",input$variable)],input$n),
